...
class BetaVAE_MLP2(nn.Module):
    ...
    def __init__(self, input_dim=3, z_dim=10, decoder_side_truez=0, decoder_side_hatz=0,decoder_side_truex=0,encoder_side_truez=0, encoder_side_truex=0, hidden_dim=128):
        super(BetaVAE_MLP2, self).__init__()
        self.z_dim = z_dim
        self.input_dim = input_dim
...
class SequentialBetaVAE_MLP(nn.Module):
    """Modified BetaVAE that uses previous frames to predict current frame."""
    
    def __init__(self, input_dim=3, z_dim=10, hidden_dim=128, num_history_frames=5):
        super(SequentialBetaVAE_MLP, self).__init__()
        self.z_dim = z_dim
        self.input_dim = input_dim
        self.num_history_frames = num_history_frames
        
        # Encoder takes concatenated history frames
        encoder_input_dim = input_dim * num_history_frames
        self.encoder = nn.Sequential(
    ...
    def forward(self, x_history, return_z=True):
        """
        Args:
            x_history: Tensor of shape (B, num_history_frames, input_dim)
        Returns:
            x_recon: Reconstruction of current frame (B, input_dim)
            mu, logvar: VAE parameters (B, z_dim)
            z: Latent representation (B, z_dim)
        """
        B, num_frames, input_dim = x_history.shape
        assert num_frames == self.num_history_frames
        assert input_dim == self.input_dim
        
        # Flatten history frames for encoder input
        x_flat = x_history.view(B, -1)  # (B, num_history_frames * input_dim)
