...
def train(args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in pbar:
            ...
            kl = losses['kl']
            independece = losses['independence']
            temporal = losses['temporal']
            ...
            if writer is not None:
                ...
                writer.add_scalar("Loss/independence", independece.item(), global_step)
                writer.add_scalar("Loss/temporal", temporal.item(), global_step)
            ...
            if is_main_process():
                pbar.set_postfix({
                    ...
                    "kl": f"{kl.item():.4f}",
                    "independence": f"{independece.item():.4f}",
                    "temporal": f"{temporal.item():.4f}"
                })
