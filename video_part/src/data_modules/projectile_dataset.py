...
class PhysicsProjectileDataset(VideoDataset):
    """
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        if self.only_latents:
            try:
                ...
                position = npz_data["position"]  
                rotation = npz_data["rotation"]  
                linear_velocity = npz_data["linear_velocity"]  
                angular_velocity = npz_data["angular_velocity"] 
                vector = self.merge_content_and_style_as_sequence(global_vec, position, rotation, linear_velocity, angular_velocity)
    ...
    def _load_npz_data(self, npz_path: str, json_metadata: Dict, frame_indices: np.ndarray) -> Dict:
        ...
        for key in ["position", "rotation", "linear_velocity", "angular_velocity"]:
            ...
            tensor = npz_data[key]  # shape: (T, D)
            T = tensor.shape[0]
            max_idx = max(frame_indices)

            if T > max_idx:
                sampled = tensor[frame_indices, ...]  # sample along time axis
            else:
    ...
    def encode_global_to_vector(self, data):
        """
        Encodes JSON global data into a feature vector using index instead of one-hot for scene and object.
        ...
        vector.append(scene_index)  # Single integer index
        ...
        if scene.get("requires_lighting", False):
            ...
            vector.extend(lighting.get("position", [0, 0, 0, 0]))
            vector.extend(lighting.get("color", [0, 0, 0]))
        else:
        ...
        object_index = objects.index(asset_name) if asset_name in objects else len(objects)
        vector.append(object_index)  # Single integer index
    ...
    def merge_content_and_style_as_sequence(self,
        ...
        dynamic_vars = [var.reshape(var.shape[0], -1) if var.ndim == 1 else var for var in dynamic_vars]
