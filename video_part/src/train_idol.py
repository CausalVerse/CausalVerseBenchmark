...
def train(args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        model.train()
        total_loss, total_recon, total_sparsity_loss, total_kld_normal, total_kld_future = 0, 0, 0, 0, 0
        ...
        for batch in pbar:
            ...
            optimizer.zero_grad()
            _, recon_x, causal_mu, causal_logvar, causal_z, z_full, _ = model(x)
            base_model = model.module if isinstance(model, DDP) else model
            recon_loss = F.mse_loss(recon_x, x, reduction="mean")
            sparsity_loss, kld_normal, kld_future = base_model.idol_process.loss_function(
                causal_mu,
                causal_logvar,
                causal_z,
            )
            loss = (
                base_model.recon_weight * recon_loss
                + base_model.idol_process.beta * kld_normal
                + base_model.idol_process.gamma * kld_future
                + base_model.idol_process.theta * sparsity_loss
            )
        ...
        if writer is not None:
            ...
            writer.add_scalar("Epoch/sparsity_loss", avg_sparsity_loss, epoch)
            writer.add_scalar("Epoch/kld_normal", avg_kld_normal, epoch)
            writer.add_scalar("Epoch/kld_future", avg_kld_future, epoch)
        if is_main_process():
            print(f"[Epoch {epoch}] loss: {avg_loss:.4f}, recon: {avg_recon:.4f}, sparsity_loss: {avg_sparsity_loss:.4f}, kld_normal: {avg_kld_normal:.4f}, avg_kld_future: {avg_kld_future:.4f}")
        if is_main_process():
...
if __name__ == "__main__":
    ...
    parser.add_argument("--latent_dim", type=int, default=2048, help="Dimensionality of the shared frame latent")
    parser.add_argument("--z_dim", type=int, default=8, help="Dimensionality of causal latent")
