...
def train(args):
    ...
    if not args.dataset_name.endswith("_vid"):
        model = TCLLatentFrameVAE(channel=4, time=args.num_frames, z_dim=args.z_dim, segment_size=args.segment_size).to(device)
    else:
        model = TCLLatentFrameVAE(channel=16, time=args.num_frames, z_dim=args.z_dim, segment_size=args.segment_size).to(device)
    if args.fp16:
        model = model.half()
    if is_distributed():
        model = DDP(
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        model.train()
        total_loss, total_recon, total_kl, total_tcl = 0, 0, 0, 0
        ...
        for batch in pbar:
            ...
            recon_x, mu, logvar, z, logits, labels = model(x)
            loss, recon_loss, kl_loss, tcl_loss = model.compute_loss(
                x,
                recon_x,
                mu,
                logvar,
            ...
            if writer is not None:
                ...
                writer.add_scalar("Loss/tcl", tcl_loss.item(), global_step)

            total_loss += loss.item()
            total_recon += recon_loss.item()
            total_kl += kl_loss.item()
            total_tcl += tcl_loss.item()
            ...
            if is_main_process() and global_step % args.save_interval == 0:
                ...
                with torch.no_grad():
                    ...
                    save_path = os.path.join(args.log_dir, f"step_{step_str}.mp4")
                    gt_save_path = os.path.join(args.log_dir, f"step_{step_str}_gt.mp4")
                    ...
                    if not args.dataset_name.endswith("_vid"):
                        ...
                        if orig_latent.shape[1] != 16 and orig_latent.shape[0] == 16:
                            orig_latent = orig_latent.permute(1, 0, 2, 3)
                        orig_latent = orig_latent.permute(1, 0, 2, 3).unsqueeze(0)
        ...
        avg_tcl = total_tcl / len(dataloader)
        if writer is not None:
            writer.add_scalar("Epoch/loss", avg_loss, epoch)
            writer.add_scalar("Epoch/reconstruction", avg_recon, epoch)
            writer.add_scalar("Epoch/KL", avg_kl, epoch)
            writer.add_scalar("Epoch/TCL", avg_tcl, epoch)
        if is_main_process():
            print(f"[Epoch {epoch}] loss: {avg_loss:.4f}, recon: {avg_recon:.4f}, kl: {avg_kl:.4f}, tcl: {avg_tcl:.4f}")
        if is_main_process():
            ckpt_path = os.path.join(args.log_dir, "checkpoints", f"vae_epoch{epoch}.pth")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_name", type=str, default="fall", help="Name of the dataset")
    ...
    parser.add_argument("--z_dim", type=int, default=10, help="Dimensionality of latent code")
    parser.add_argument("--log_dir", type=str, default="training-runs/tcl/test")
    parser.add_argument("--save_interval", type=int, default=10000, help="Interval (in steps) to save reconstructed videos")
    ...
    parser.add_argument("--tcl_weight", type=float, default=0.1)
